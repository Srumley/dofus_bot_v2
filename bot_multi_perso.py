...
while True:
    ...
    if i ==2: player=player3
    print(player.name)
    if player.nb_cut == 0:
        time_cutting = 4+8
    else: time_cutting = 6

    if time.time()-time_cutting > player.timer_begin_cutting and time.time()-8 > player.timer_begin_moving:
        if time.time()-time_cutting > player.timer_begin_cutting:
            player.is_cutting = False

        if time.time()-8> player.timer_begin_moving:
            player.is_moving = False

        if player.difference_position[0]==0 and player.difference_position[1]==0 and player.empty_map==False:
            player.in_target_map = True
        ...
        player.checkFullPod()
        if player.full_pod==True:
            player.target_map = [4, -20]
            player.moving_target()
            if player.actuel_map == player.target_map:
                sleep(6)
                pg.click(down)
                sleep(6)
                pg.click(down)
                ...
                print('in map bank')
                #sleep(3)
                player.go_bank_astrub()
                sleep(3)
                pg.click(up)
                # player.difference_position[1] = player.difference_position[1] - 1
                # player.actuel_map[1] = player.actuel_map[1] + 1
                sleep(6)
                pg.click(up)
                sleep(6)

                player.full_pod = False
                if player == player1:
                    player.target_map = player.tour_astrub_ble1[player.idx_tour]
                if player == player2:
                    player.target_map = player.tour_astrub_ble2[player.idx_tour]
                if player == player3:
                    player.target_map = player.tour_astrub_ble2[player.idx_tour]
                player.moving_target()

        if player.is_cutting==False and player.is_moving==False and player.full_pod==False:
            if player.in_target_map==False:
                if player == player1:
                    player.target_map = player.tour_astrub_ble1[player.idx_tour]
                if player == player2:
                    player.target_map = player.tour_astrub_ble2[player.idx_tour]
                if player == player3:
                    player.target_map = player.tour_astrub_ble2[player.idx_tour]
                player.moving_target()
                player.empty_map = False

            if player.in_target_map==True:
                print('collecting')
                player.collect()
